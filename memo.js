...
eval()             // Встроенная функция eval позволяет выполнять строку кода (например "1"+"1" => 2, а не 11)
    ...
    .slice(a, b)   // возвращает часть строки, обьекта, массива по заданным инд-м('a'начало(вкл.)-'b'конец(не вкл.))
    ...
    .test(str)     //выполняет поиск сопоставления регулярного выражения указанной строке. Возвращает true или false
    .endsWith()    //если строка заканчивается символами указанными в скобках, возвращает true, иначе false

    .replace()     //возвращает новую строку с некоторыми или всеми сопоставлениями с шаблоном => пример =>
/*  const re = /яблоки/gi;  // g = глобальный i = игнорировать регистр
const str = "Яблоки круглые и яблоки сочные.";
const newstr = str.replace(re, "апельсины");
...
/* Array.from(obj1) принимает объект, проверяет, является ли он итерируемым
объектом или псевдомассивом, затем создаёт массив, копирует туда все элементы и возвр-т его.
...
arr.pop()       // удаляет эл-т из конца массива и возвращает его
...
delete arr[i]   // удаляет эл-т по индексу
arr.splice(index, count, elem)    /* удалить count эл-ов, начиная с index
...
arr.forEach(e => { // перебирает м-в и вызывает функцию для каждого элемента массива 
    e()
});
...
arr.find() /*возвращает значение первого найденного в массиве элемента, которое удовлетворяет
условию переданному в callback функции.В противном случае возвращается undefined. */
arr.includes() // определяет, содержит ли массив определённый элемент,
// возвращая в зависимости от этого true или false
arr.indexOf() // возвращает первый индекс, по которому данный эл.может быть найден в массиве
// или -1, если такого индекса нет.
arr.fill(value, startIndex, finishIndex) // заполняет все элементы массива от начального 
//до конечного индексов одним значением.
arr.some(() => { }) // проверяет, проходит ли хотя бы один элемент массива тест,
// реализованный предоставленной функцией. Возвращает true/false
...
let { title: t, width: w, height: h } = options; // переприсвоение значений ключей 
// переменным. т.е. (let t="Menu",let w=100,let h=200)
